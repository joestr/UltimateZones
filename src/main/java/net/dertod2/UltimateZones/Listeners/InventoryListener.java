...
public class InventoryListener implements Listener {
    ...
    public void onInventory(InventoryOpenEvent event) {
        if (!(event.getPlayer() instanceof Player))
            return;
        if (!isCheckableInventory(event.getInventory().getType()))
            return;
    ...
    public void onInventoryAction(InventoryActionEvent event) {
        if (!(event.getPlayer() instanceof Player))
            return;
        if (!InventoryListener.isCheckableInventory(event.getInventory().getType()))
            return;
        ...
        if (event.isToContainer()) {
            RightResult rightResult = abstractZone.hasRight((Player) event.getPlayer(), Flag.ContainerPut,
                    event.getToContainer().getType());
            if (rightResult.rightEnum != RightEnum.Success) {
                MessageUtils.sendFailed(abstractZone, (Player) event.getPlayer(), rightResult);
                event.setCancelled(true);
            }
        ...
        if (event.isToEntity()) {
            RightResult rightResult = abstractZone.hasRight((Player) event.getPlayer(), Flag.ContainerGet,
                    event.getToEntity().getType());
            if (rightResult.rightEnum != RightEnum.Success) {
                MessageUtils.sendFailed(abstractZone, (Player) event.getPlayer(), rightResult);
                event.setCancelled(true);
            }
    ...
    private static boolean isCheckableInventory(InventoryType inventoryType) {
        if (inventoryType.equals(InventoryType.ANVIL))
            return false;
        if (inventoryType.equals(InventoryType.BEACON))
            return false;
        if (inventoryType.equals(InventoryType.CRAFTING))
            return false;
        if (inventoryType.equals(InventoryType.CREATIVE))
            return false;
        if (inventoryType.equals(InventoryType.ENCHANTING))
            return false;
        if (inventoryType.equals(InventoryType.ENDER_CHEST))
            return false;
        if (inventoryType.equals(InventoryType.MERCHANT))
            return false;
        if (inventoryType.equals(InventoryType.PLAYER))
            return false;
        if (inventoryType.equals(InventoryType.WORKBENCH))
            return false;
