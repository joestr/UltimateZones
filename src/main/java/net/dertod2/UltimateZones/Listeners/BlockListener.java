...
public class BlockListener implements Listener {
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		RightResult rightResult = abstractZone.hasRight(event.getPlayer(), Flag.Place, event.getBlockPlaced().getType());
	...
	public void onBlockDestroy(BlockBreakEvent event) {
		...
		RightResult rightResult = abstractZone.hasRight(event.getPlayer(), event.getBlock() instanceof InventoryHolder ? Flag.BreakContainer : Flag.Break, event.getBlock().getType());
		if (rightResult.rightEnum != RightEnum.Success) {
			MessageUtils.sendFailed(abstractZone, event.getPlayer(), rightResult);
			event.setCancelled(true);
		}
	...
	public void onBlockPistonExtend(BlockPistonExtendEvent event) {
		...
		for (int i = 0; i < event.getBlocks().size() + 2; i++)  blockList.add(event.getBlock().getRelative(event.getDirection(), i));
		
		for (Block block : blockList) {
			AbstractZone blockZone = AbstractZone.getZone(block.getLocation());

			if (pistonZone == null && blockZone == null) continue;
			if (pistonZone != null && blockZone == null) continue;
			if (pistonZone != null &&  blockZone != null && pistonZone.getId() == blockZone.getId()) continue;
			// TODO Check if Zones having the same owner & parent -> problem with cities -> extra Zone Setting/Flag ?
	...
	public void onBlockPistonRetract(BlockPistonRetractEvent event) {
		if (event.getRetractLocation().getBlock().isEmpty() || !event.isSticky()) return;

		Block block = event.getRetractLocation().getBlock();

		AbstractZone retractZone = AbstractZone.getZone(block.getLocation());
		if (retractZone == null) return;
		...
		if (targetZone == null || retractZone.getId() != targetZone.getId()) event.setCancelled(true);
	}
}