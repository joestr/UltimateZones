...
public class FindListener implements Listener {
    ...
    public void onRightClick(PlayerInteractEvent event) {
        if (event.getAction() != Action.RIGHT_CLICK_BLOCK)
            return;
        if (!FindCommand.findList.contains(event.getPlayer().getUniqueId()))
            return;

        if (UltimateZones.findItem == null || UltimateZones.findItem.getType() == Material.AIR
                || UltimateZones.findItem.getType() == event.getMaterial()) {
            ...
            if (abstractZone != null) {
                ...
                if (abstractZone instanceof SubZone) {
                    ...
                    Locale.json(event.getPlayer(), "find.subzone", synonym, abstractZone.getId(),
                    ...
                    Locale.json(event.getPlayer(), "find.mainzone", synonym, abstractZone.getId(),
                            abstractZone.getPlayer().getName());
                }
            } else {
