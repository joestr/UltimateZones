...
public class EntityListener implements Listener {
	...
	public void onHangingEntityDestroy(HangingBreakByEntityEvent event) {
		...
		RightResult rightResult = abstractZone.hasRight(player, Flag.Break, UltimateUtils.getItemType(event.getEntity()));
		if (rightResult.rightEnum != RightEnum.Success) {
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		if (event.getEntity() instanceof Animals || event.getEntity() instanceof Golem || event.getEntity() instanceof Villager) {
			...
			RightResult rightResult = abstractZone.hasRight(player, itemStack == null || itemStack.getType() == Material.AIR ? Flag.Break : Flag.ContainerGet); // TODO null check ?
			if (rightResult.rightEnum != RightEnum.Success) {
				MessageUtils.sendFailed(abstractZone, player, rightResult);
				event.setCancelled(true);
			}
	...
	public void onSheepRightClick(PlayerInteractEntityEvent event) {
		...
		if (!UltimateUtils.equalsHandItem(event.getPlayer(), new MaterialData(Material.SHEARS))) return;
	...
	public void onEntityFeed(PlayerInteractEntityEvent event) {
		if (!(event.getRightClicked() instanceof Ageable)) return;
		if (!UltimateUtils.isCorrectFeedingItem((Ageable) event.getRightClicked(), event.getPlayer().getItemInHand())) return;
	...
	public void onItemFrameRotate(PlayerInteractEntityEvent event) {
		...
		ItemStack itemStack = itemFrame.getItem();
		RightResult rightResult = abstractZone.hasRight(event.getPlayer(), itemStack == null || itemStack.getType() == Material.AIR ? Flag.ContainerPut : Flag.RotateFrames); // TODO null check ?
		if (rightResult.rightEnum != RightEnum.Success) {
			MessageUtils.sendFailed(abstractZone, event.getPlayer(), rightResult);
			event.setCancelled(true);
		}
