...
public class PlayerListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        //if (UltimateZones.updater.updateAvailable())
        //    UltimateZones.updater.check(event.getPlayer());
    }

    @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true)
    public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event) {
        AbstractZone abstractZone = AbstractZone
                .getZone(event.getBlockClicked().getRelative(event.getBlockFace()).getLocation());
        if (abstractZone == null)
            return;

        RightResult rightResult = abstractZone.hasRight(event.getPlayer(), Flag.Place,
                event.getBucket() == Material.WATER_BUCKET ? Material.WATER : Material.LAVA);
    ...
    public void onPlayerBucketFill(PlayerBucketFillEvent event) {
        ...
        if (event.getBucket() == Material.MILK_BUCKET) {
            ...
            RightResult rightResult = abstractZone.hasRight(event.getPlayer(), Flag.Break,
                    material == Material.WATER || material == Material.LEGACY_STATIONARY_WATER ? Material.WATER
                            : Material.LAVA);
            if (rightResult.rightEnum != RightEnum.Success) {
                MessageUtils.sendFailed(abstractZone, event.getPlayer(), rightResult);
                event.setCancelled(true);
    ...
    public void onItemPickup(PlayerPickupItemEvent event) {
        ...
        RightResult rightResult = abstractZone.hasRight(event.getPlayer(), Flag.PickupItems,
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if (!UltimateZones.getConfiguration().getStringList("sethome-commands").contains(command))
            return;
    ...
    public void onZoneInfo(ZoneInfoEvent event) {
        ...
        if (abstractZone.hasSynonym())
            event.addInformation("synonym", abstractZone.getSynonym());
