...
public class NormalCreation extends AbstractCreation {
    ...
    public NormalCreation(Player zoneCreator, UltimateZoneType ultimateZoneType, ZoneType zoneType, boolean force3d) {
        ...
        if (zoneType.getMaximum() != -1)
            for (int i = 0; i < zoneType.getMaximum(); i++)
                this.positions.add(null);
    }

    public NormalCreation(Player zoneCreator, OfflinePlayer zoneOwner, UltimateZoneType ultimateZoneType,
            ZoneType zoneType, boolean force3d) {
    ...
    public void setLocation(PlayerInteractEvent event) {
        ...
        case CUBOID:
            if (rightClick) {
                if (this.positions.get(0) == null || !this.positions.get(0).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.cuboid.right", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
                    this.positions.set(0, location);
                }
            } else {
                if (this.positions.get(1) == null || !this.positions.get(1).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.cuboid.left", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
        ...
        case CYLINDER:
            if (rightClick) {
                if (this.positions.get(0) == null || !this.positions.get(0).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.cylinder.right", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
                    this.positions.set(0, location);
                }
            } else {
                if (this.positions.get(1) == null || !this.positions.get(1).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.cylinder.left", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
                    this.positions.set(1, location);
                }
            }
        ...
        case SPHERE:
            if (rightClick) {
                if (this.positions.get(0) == null || !this.positions.get(0).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.sphere.right", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
                    this.positions.set(0, location);
                }
            } else {
                if (this.positions.get(1) == null || !this.positions.get(1).equals(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.sphere.left", location.getBlockX(),
                            location.getBlockY(), location.getBlockZ());
                    this.positions.set(1, location);
                }
            }
        ...
        case POLYGON:
            if (rightClick) {
                if (!this.positions.contains(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.polygon.right.success",
                            this.getSize(this.positions) + 1, location.getBlockX(), location.getBlockY(),
                            location.getBlockZ());
                ...
                if (this.positions.remove(location)) {
                    Locale.sendPlain(this.zoneCreator, "creation.normal.positions.polygon.left.success",
        ...
        if (this.getSize(this.positions) < this.zoneType.getMinimum())
            return;
        this.creationState = this.zoneType == ZoneType.POLYGON ? CreationState.CheckPossible
                : CreationState.WaitForCheck;
        ...
        if (previewZone instanceof OriginZone) {
            ...
            Locale.sendPlain(this.zoneCreator, "creation.normal.info.origin", origin.getBlockX(), origin.getBlockY(),
                    origin.getBlockZ());
        }

        Locale.sendPlain(this.zoneCreator, "creation.normal.info.area",
                UltimateUtils.formattedArea(previewZone.getArea(), Locale.get(this.zoneCreator).getJavaLocale()));
        Locale.sendPlain(this.zoneCreator, "creation.normal.info.volume",
                UltimateUtils.formattedVolume(previewZone.getVolume(), Locale.get(this.zoneCreator).getJavaLocale()));

        Locale.json(this.zoneCreator, "creation.normal.info.clickhere");
    }

    public boolean check(boolean silent) {
        if (this.creationState != CreationState.WaitForCheck && this.creationState != CreationState.CheckPossible
        ...
        if (this.ultimateZoneType == UltimateZoneType.MAIN) {
            if (!ignoreRestrictions) {
                ...
                if (maxZones != -1 && AbstractZone.getZones(this.zoneOwner).size() + 1 > maxZones) {
                    if (!silent)
                        Locale.sendPlain(this.zoneCreator, "", maxZones);
                    return false;
                }
                ...
                if (checkZone instanceof HeightZone && this.force3D) {
                    ...
                    if (minHeight != -1 && height < minHeight) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.heightzone.minheight", minHeight);
                        return false;
                    }

                    if (maxHeight != -1 && height > maxHeight) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.heightzone.maxheight", maxHeight);
                        return false;
                    }
                }
                ...
                if (checkZone instanceof RoundedZone) {
                    ...
                    if (minRadius != -1 && radius < minRadius) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.roundedzone.minradius",
                                    minRadius);
                        return false;
                    }
                    ...
                    if (maxRadius != -1 && radius > maxRadius) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.roundedzone.maxradius",
                                    maxRadius);
                        return false;
                    }
                }
                ...
                case CUBOID:
                    ...
                    if (minWidth != -1 && zoneWidth < minWidth || zoneLength < minWidth) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.cuboid.minlength", minWidth);
                        return false;
                    }

                    if (maxWidth != -1 && (zoneWidth > maxWidth || zoneLength > maxWidth)) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.cuboid.maxlength", maxWidth);
                        return false;

                    }

                    if (aspectRatio != -1
                            && (zoneLength > (aspectRatio * zoneWidth) || zoneWidth > (aspectRatio * zoneLength))) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.cuboid.aspect", aspectRatio);
                        return false;
                    }
                ...
                case POLYGON:
                    ...
                    if (maxDots != -1 && dots > maxDots) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.polygon.maxdots", maxDots);
                        return false;
                    }
                ...
                if (maxArea != -1) {
                    ...
                    if (completeArea > maxArea) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.main.maxarea", maxArea);
                        return false;
                    }
                }
                ...
                if (maxVolume != -1) {
                    ...
                    if (completeVolume > maxVolume) {
                        if (!silent)
                            Locale.sendPlain(this.zoneCreator, "creation.normal.check.main.maxvolume", maxVolume);
                        return false;
                    }
                }
            }
            ...
            for (AbstractZone abstractZone : collisionList) {
                ...
                if (disableOverlapping || this.zonePriority == mainZone.getPriority()) {
                    if (!silent)
                        Locale.json(this.zoneCreator, "creation.normal.check.collision.main",
                                mainZone.hasSynonym() ? mainZone.getSynonym() : mainZone.getId(), mainZone.getId(),
                                mainZone.getPlayer().getName());
                    if (!ignoreRestrictions)
                        return false;
                }
            }
            ...
            if (!ignoreRestrictions) {
                ...
                if (maxChilds != -1 && this.parentZone.getZones(true).size() + 1 > maxChilds) {
                    if (!silent)
                        Locale.sendPlain(this.zoneCreator, "creation.normal.check.child.maxchilds", maxChilds);
                    return false;
                }

                if (maxDepth != -1 && UltimateUtils.getRecursiveZoneDepth(this.parentZone) + 1 > maxDepth) {
                    if (!silent)
                        Locale.sendPlain(this.zoneCreator, "creation.normal.check.child.maxdepth", maxDepth);
                    return false;
                }

                if (!this.parentZone.isOwner((CommandSender) this.zoneCreator) && this.parentZone
                        .hasRight(this.zoneCreator, Flag.AdministrateZones).rightEnum != RightEnum.Success) {
            ...
            for (SubZone subZone : zoneList) {
                if (subZone.getZone().intersect(checkZone)) {
                    if (!silent)
                        Locale.json(this.zoneCreator, "creation.normal.check.collision.child",
                                subZone.hasSynonym() ? subZone.getSynonym() : subZone.getId(), subZone.getId(),
                                subZone.getPlayer().getName());
                    return false;
                }
            }
        }
        ...
        if (!silent)
            Locale.json(this.zoneCreator, "creation.normal.check.success");
    ...
    public boolean create() {
        if (this.creationState != CreationState.CheckSuccesfully)
            return false;
        if (!this.check(true))
            return false; // To be safe <- Check if other zones has been created meanwhile

        Zone zone = Zone.create(this.zoneCreator, this.cleanup(this.positions), !this.force3D, this.zoneType);
        if (zone != null) {
            AbstractZone abstractZone = UltimateZones.zoneControl.addZone(zone, this.ultimateZoneType, this.zoneOwner,
                    this.parentZone != null ? this.parentZone.getId() : -1, this.zonePriority);
    ...
    public void info() {
        if (UltimateZones.zoneItem != null && UltimateZones.zoneItem.getType() != Material.AIR) {
            Locale.json(this.zoneCreator, "creation.normal.info.withitem",
                    NMSHelper.getMojangIdentifierName(UltimateZones.zoneItem),
                    NMSHelper.getMojangTranslatableName(UltimateZones.zoneItem));
        } else {
