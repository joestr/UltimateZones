...
public enum CreationMethod {
	/**
	 * The normal Creation, the User must do the creation complete without an automatic creation wizard
	 */
	Normal(NormalCreation.class, ImmutableList.<ZoneType>of(ZoneType.CUBOID, ZoneType.CYLINDER, ZoneType.SPHERE, ZoneType.POLYGON));
	...
	public static CreationMethod get(Class<? extends AbstractCreation> wizardClass) {
		for (CreationMethod creationMethod : CreationMethod.values()) {
			if (wizardClass.isAssignableFrom(creationMethod.wizardClass)) return creationMethod;
		}
	...
	public static CreationMethod get(String creationMethodName) {
		for (CreationMethod creationMethod : CreationMethod.values()) {
			if (creationMethodName.equals(creationMethod.name())) return creationMethod;
		}
