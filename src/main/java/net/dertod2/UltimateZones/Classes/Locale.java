...
public class Locale {
    ...
    public Locale(List<NodeList> nodes) throws Exception {
        ...
        for (NodeList nodeList : nodes) {
            ...
            boolean parseColors = localeElement.getElementsByTagName("colors").item(0).getTextContent()
            ...
            for (int i = 0; i < entries.getLength(); i++) {
                ...
                if (parseColors && phrase.contains("§")) {
                    for (ChatColor chatColor : ChatColor.values()) {
                        phrase = phrase.replace("§" + chatColor.name().toLowerCase().replace("_", ""),
                                "§" + chatColor.getChar());
                    }
                }
    ...
    private void addPhrase(String module, String path, String phrase) {
        if (!this.phrases.containsKey(module))
            this.phrases.put(module, new HashMap<String, String>());
