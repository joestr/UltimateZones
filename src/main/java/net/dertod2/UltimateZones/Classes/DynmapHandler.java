...
public class DynmapHandler {
    ...
    private static Map<Integer, MarkerDescription> markerList; // Do not init static cause the class MarkerDescription
                                                               // can be not existing
    ...
    public DynmapHandler() {
        ...
        DynmapHandler.dateFormat = new SimpleDateFormat(
                UltimateZones.getConfiguration().getString("date-layouts." + DynmapHandler.locale.getTag()));
        ...
        if (DynmapHandler.mainSet != null)
            DynmapHandler.mainSet.deleteMarkerSet(); // To always have an valid layout, like database changes

        if (DynmapHandler.showMainSet) {
            DynmapHandler.mainSet = DynmapHandler.markerAPI.createMarkerSet("ultimatezones.main",
                    DynmapHandler.locale.parse("dynmap", "markersets.mainzones"), null, false);
        ...
        if (DynmapHandler.childSet != null)
            DynmapHandler.childSet.deleteMarkerSet(); // To always have an valid layout, like database changes

        if (DynmapHandler.showChildSet) {
            DynmapHandler.childSet = DynmapHandler.markerAPI.createMarkerSet("ultimatezones.child",
                    DynmapHandler.locale.parse("dynmap", "markersets.childzones"), null, false);
        ...
        if (DynmapHandler.serverSet != null)
            DynmapHandler.serverSet.deleteMarkerSet(); // To always have an valid layout, like database changes

        if (DynmapHandler.showServerSet) {
            DynmapHandler.serverSet = DynmapHandler.markerAPI.createMarkerSet("ultimatezones.server",
                    DynmapHandler.locale.parse("dynmap", "markersets.serverzones"), null, false);
    ...
    public static void addZone(AbstractZone abstractZone) {
        ...
        if (!DynmapHandler.showMainSet && abstractZone instanceof MainZone)
            return;
        if (!DynmapHandler.showChildSet && abstractZone instanceof SubZone)
            return;
        if (UltimateZones.getConfiguration().getStringList("dm-ignored-worlds").contains(zone.getWorldName()))
            return;
        ...
        if (UltimateZones.getConfiguration().getBoolean("dm-activate-server-set")) {
            if (UltimateZones.getConfiguration().getStringList("dm-server-set-players")
                    .contains(abstractZone.getPlayer().getName())) {
                markerSet = DynmapHandler.serverSet;
            }
        }

        String infoWindow = "<div class=\"regioninfo\">" + (zone instanceof CuboidZone ? DynmapHandler.cuboidDescription
                : zone instanceof CylinderZone ? DynmapHandler.cylinderDescription
                        : zone instanceof SphereZone ? DynmapHandler.sphereDescription
                                : DynmapHandler.polygonDescription)
                + "</div>";
        ...
        if (zone.getCreated() != null && DynmapHandler.dateFormat != null)
            infoWindow = infoWindow.replace("%created%", DynmapHandler.dateFormat.format(zone.getCreated()));
        infoWindow = infoWindow.replace("%area%",
                UltimateUtils.formattedArea(zone.getArea(), DynmapHandler.locale.getJavaLocale()));
        infoWindow = infoWindow.replace("%volume%",
                UltimateUtils.formattedVolume(zone.getVolume(), DynmapHandler.locale.getJavaLocale()));

        MarkerDescription markerDescription = null;

        if (zone instanceof HeightZone) {
            infoWindow = infoWindow.replace("%height%",
                    ((HeightZone) zone).isFullHeight()
                            ? DynmapHandler.locale.parse("dynmap", "descriptions.fullheightzone")
                            : decimalFormat.format(((HeightZone) zone).getHeight()));
        }
        ...
        if (zone instanceof RoundedZone) {
            ...
            CircleMarker circleMarker = markerSet.createCircleMarker(String.valueOf(abstractZone.getId()),
                    "Zone: " + abstractZone.getId(), false, zone.getWorld().getName(), ((OriginZone) zone).getX() + .5,
                    ((OriginZone) zone).getY(), ((OriginZone) zone).getZ() + .5, radius, radius, true);

            if (abstractZone instanceof MainZone) {
                circleMarker.setLineStyle(UltimateZones.getConfiguration().getInt("dm-main-weight"),
                        UltimateZones.getConfiguration().getDouble("dm-main-lineopacity"),
                        UltimateZones.getConfiguration().getInt("dm-main-linecolor"));
                circleMarker.setFillStyle(UltimateZones.getConfiguration().getDouble("dm-main-fillopacity"),
                        UltimateZones.getConfiguration().getInt("dm-main-fillcolor"));
            } else {
                circleMarker.setLineStyle(UltimateZones.getConfiguration().getInt("dm-child-weight"),
                        UltimateZones.getConfiguration().getDouble("dm-child-lineopacity"),
                        UltimateZones.getConfiguration().getInt("dm-child-linecolor"));
                circleMarker.setFillStyle(UltimateZones.getConfiguration().getDouble("dm-child-fillopacity"),
                        UltimateZones.getConfiguration().getInt("dm-child-fillcolor"));
            }
            ...
            PolyLineMarker polygonMarker = markerSet.createPolyLineMarker(String.valueOf(abstractZone.getId()),
                    "Zone: " + abstractZone.getId(), false, zone.getWorld().getName(), polysX, polysY, polysZ, true);

            polygonMarker.setLineStyle(UltimateZones.getConfiguration().getInt("dm-main-weight") + 1,
                    UltimateZones.getConfiguration().getDouble("dm-main-lineopacity"),
                    UltimateZones.getConfiguration().getInt("dm-main-linecolor"));
            ...
            AreaMarker areaMarker = markerSet.createAreaMarker(String.valueOf(abstractZone.getId()),
                    "Zone: " + abstractZone.getId(), false, zone.getWorld().getName(),
                    new double[] { min.getX(), max.getX() }, new double[] { min.getZ(), max.getZ() }, true);

            if (abstractZone instanceof MainZone) {
                areaMarker.setLineStyle(UltimateZones.getConfiguration().getInt("dm-main-weight"),
                        UltimateZones.getConfiguration().getDouble("dm-main-lineopacity"),
                        UltimateZones.getConfiguration().getInt("dm-main-linecolor"));
                areaMarker.setFillStyle(UltimateZones.getConfiguration().getDouble("dm-main-fillopacity"),
                        UltimateZones.getConfiguration().getInt("dm-main-fillcolor"));
            } else {
                areaMarker.setLineStyle(UltimateZones.getConfiguration().getInt("dm-child-weight"),
                        UltimateZones.getConfiguration().getDouble("dm-child-lineopacity"),
                        UltimateZones.getConfiguration().getInt("dm-child-linecolor"));
                areaMarker.setFillStyle(UltimateZones.getConfiguration().getDouble("dm-child-fillopacity"),
                        UltimateZones.getConfiguration().getInt("dm-child-fillcolor"));
            }
    ...
    public static void deleteZone(AbstractZone abstractZone) {
        ...
        if (markerDescription != null)
            markerDescription.deleteMarker();
    }
}