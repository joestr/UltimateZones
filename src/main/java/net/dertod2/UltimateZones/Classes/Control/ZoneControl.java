...
public class ZoneControl {
	...
	private Map<UUID, Map<String, Integer>> fastSynonymList; // Contains fast access over unique User UUID and Synonym for Zones
	...
	public void load() {
		if (this.zoneList == null) this.zoneList = new HashMap<Integer, AbstractZone>();
		if (this.fastZoneList == null) this.fastZoneList = new HashMap<Integer, Integer>();
		if (this.fastPlayerList == null) this.fastPlayerList = new HashMap<UUID, List<Integer>>();
		if (this.fastSynonymList == null) this.fastSynonymList = new HashMap<UUID, Map<String, Integer>>();
		...
		try {
			...
			for (TableEntry tableEntry : dataList) {
				...
				if (abstractZone.getZone() == null) {
					UltimateUtils.error("Zone " + ChatColor.GOLD +  abstractZone.getId() + ChatColor.RED + " is no longer valid. Skipped Zone!");
					continue;
				}
				
				if (abstractZone.getZone().getWorld() == null || UltimateZones.getConfiguration(abstractZone.getZone().getWorldName()) == null) {
					if (!skippedMissingConfig.contains(abstractZone.getZone().getWorldName())) skippedMissingConfig.add(abstractZone.getZone().getWorldName());
					continue;
				}
				
				if (!UltimateZones.getConfiguration(abstractZone.getZone().getWorldName()).getBoolean("activate-zones", true)) {
					if (!skippedDeactivated.contains(abstractZone.getZone().getWorldName())) skippedDeactivated.add(abstractZone.getZone().getWorldName());
					continue;
				}
				...
				if (abstractZone.hasSynonym()) {
					if (!this.fastSynonymList.containsKey(uniqueId)) this.fastSynonymList.put(uniqueId, new HashMap<String, Integer>());
					this.fastSynonymList.get(uniqueId).put(abstractZone.getSynonym(), abstractZone.getId());
				}
				
				if (abstractZone instanceof MainZone) {
					if (!this.fastPlayerList.containsKey(uniqueId)) this.fastPlayerList.put(uniqueId, new ArrayList<Integer>());
					this.fastPlayerList.get(uniqueId).add(abstractZone.getId());
				} else {
					((SubZone) abstractZone).getParent().subList.add(abstractZone.getId());
				}
				
				if (UltimateZones.isDynmapInstalled) DynmapHandler.addZone(abstractZone);
			}
			
			if (skippedMissingConfig.size() > 0) {
				for (String worldName : skippedMissingConfig) {
					UltimateUtils.error("Skipped zones of world " + ChatColor.GOLD + worldName + ChatColor.RED + " cause of missing world or world configuration.");
				}				
			}
			
			if (skippedDeactivated.size() > 0) {
				for (String worldName : skippedMissingConfig) {
					UltimateUtils.notify("Ignored zones of world " + ChatColor.GOLD + worldName + ChatColor.RED + " cause of 'activate-zones' set to false.");
				}				
			}
			
			UltimateUtils.notify("Loaded and initialized " + ChatColor.GOLD + this.zoneList.size() + ChatColor.DARK_GREEN + " zones!");
		}
	}
	...
	public void move(AbstractZone abstractZone, UUID oldPlayer, UUID newPlayer) {
		...
		if (!this.fastPlayerList.containsKey(newPlayer)) this.fastPlayerList.put(newPlayer, new ArrayList<Integer>());
		this.fastPlayerList.get(newPlayer).add(abstractZone.getId());
		
		if (abstractZone.hasSynonym()) {
			this.fastSynonymList.remove(oldPlayer);
			
			if (!this.fastSynonymList.containsKey(newPlayer)) this.fastSynonymList.put(newPlayer, new HashMap<String, Integer>());
			...
			for (Integer zoneId : zoneList) {
				...
				if (oldZone.hasSynonym()) this.fastSynonymList.get(oldPlayer).put(oldZone.getSynonym(), zoneId);
			}
		}
		
		if (UltimateZones.isDynmapInstalled) DynmapHandler.addZone(abstractZone); // To Update the content
	}
	...
	public AbstractZone getZone(OfflinePlayer player, String synonym) {
		...
		if (synonymList == null || synonymList.size() <= 0) return null;
	...
	public List<String> getSynonyms(OfflinePlayer offlinePlayer) {
		...
		if (synonymList == null ) return ImmutableList.<String>of();
	...
	public List<MainZone> getZones(OfflinePlayer player, World world) {
		...
		if (mainList == null) return zoneList;
		
		for (Integer zoneId : mainList) {
			AbstractZone mainZone = this.zoneList.get(zoneId);
			if (mainZone != null && (world == null || mainZone.getZone().getWorldName().equals(world.getName()))) zoneList.add((MainZone) mainZone);
		}
	...
	public AbstractZone getZone(Location location) {
		...
		if (zoneList.size() <= 0) return null;
		...
		for (AbstractZone abstractZone : zoneList) {
			...
			if (returnableZone == null) returnableZone = mainZone;
			if (mainZone.getPriority() > ((MainZone) returnableZone).getPriority()) returnableZone = mainZone;
		}
		...
		while ((childList = returnableZone.getZones(false)).size() > 0) {
			...
			if (!hasCollidedZone) break;
		}

		return returnableZone;
	}
	
	public AbstractZone addZone(Zone zone, UltimateZoneType zoneType, OfflinePlayer player, int parentId, int priority) {
		AbstractZone abstractZone = null;
		
		switch (zoneType) {
		case EXTENDED:
			abstractZone = new ExtendedZone(-1, zone.getId(), "", new ArrayList<Integer>(), new ArrayList<String>(), new ArrayList<String>(), parentId);
			break;
		case INDEPENDENT:
			abstractZone = new IndependentZone(-1, zone.getId(), "", new ArrayList<Integer>(), new ArrayList<String>(), new ArrayList<String>(), parentId);
			break;
		case MAIN:
			abstractZone = new MainZone(-1, zone.getId(), "", new ArrayList<Integer>(), new ArrayList<String>(), new ArrayList<String>(), player.getUniqueId(), priority);
			break;
		}
		...
		if (abstractZone != null) {
			...
			if (abstractZone instanceof MainZone) {
				if (!this.fastPlayerList.containsKey(player.getUniqueId())) this.fastPlayerList.put(player.getUniqueId(), new ArrayList<Integer>());
	...
	public void delZone(AbstractZone abstractZone) {
		...
		if (abstractZone.hasSynonym()) this.fastSynonymList.get(abstractZone.getPlayerUniqueId()).remove(abstractZone.getSynonym());
		
		try { abstractZone.getZone().delete(); } catch (Exception exc) { }
		
		if (UltimateZones.isDynmapInstalled) DynmapHandler.deleteZone(abstractZone);
	}
	...
	public void updateSynonym(AbstractZone abstractZone, String newSynonym) {
		...
		if (abstractZone.hasSynonym()) synonymList.remove(abstractZone.getSynonym());

		if (newSynonym.length() > 0) {
			if (synonymList == null) this.fastSynonymList.put(abstractZone.getPlayerUniqueId(), new HashMap<String, Integer>());
