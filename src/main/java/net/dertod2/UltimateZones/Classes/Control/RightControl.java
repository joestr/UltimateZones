...
public class RightControl {
    ...
    public Map<UUID, RightReference> getZoneRights(AbstractZone abstractZone) {
        ...
        try {
            DatabaseHandler.get().getHandler().load(new RightReference(), dataList,
                    ImmutableMap.<String, Object>builder().put("zone", abstractZone.getId()).build());
        } catch (Exception exc) {
            exc.printStackTrace();
        } finally {
    ...
    public List<RightReference> getPlayerRights(OfflinePlayer offlinePlayer) {
        ...
        try {
            DatabaseHandler.get().getHandler().load(new RightReference(), dataList, ImmutableMap
                    .<String, Object>builder().put("player", offlinePlayer.getUniqueId().toString()).build());
        } catch (Exception exc) {
            exc.printStackTrace();
        } finally {
            for (TableEntry tableEntry : dataList) {
    ...
    public void clearRights(AbstractZone abstractZone) {
        try {
            DatabaseHandler.get().getHandler().remove(new RightReference(),
                    ImmutableMap.<String, Object>builder().put("zone", abstractZone.getId()).build());
        } catch (Exception exc) {
            exc.printStackTrace();
        }
    ...
    public void updateRights(RightReference rightReference) {
        try {
            DatabaseHandler.get().getHandler().update(rightReference,
                    ImmutableMap.<String, Object>builder().put("player", rightReference.getPlayerUniqueIdString())
                            .put("zone", rightReference.getZoneId()).build());
        } catch (Exception exc) {
            exc.printStackTrace();
        }
    }
    ...
    public void deleteRights(RightReference rightReference) {
        try {
            DatabaseHandler.get().getHandler().remove(rightReference,
                    ImmutableMap.<String, Object>builder().put("player", rightReference.getPlayerUniqueIdString())
                            .put("zone", rightReference.getZoneId()).build());
        } catch (Exception exc) {
            exc.printStackTrace();
