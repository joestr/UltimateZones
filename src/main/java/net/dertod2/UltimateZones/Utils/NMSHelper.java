...
public class NMSHelper {
	...
	public static String getMojangTranslatableName(ItemStack itemStack) {
		...
		if (nmsStack == null) {
			...
			if (item == null) {
				...
				if (block != null) {
					foundType = net.minecraft.server.v1_8_R3.LocaleI18n.get(block.a() + ".name");
				}		
			} else {
				foundType = net.minecraft.server.v1_8_R3.LocaleI18n.get(item.getName() + ".name");
			}
		} else {
			foundType = nmsStack.getItem().k(nmsStack) + ".name";
		}
	...
	public static String getMojangIdentifierName(ItemStack itemStack) {
		...
		if (nmsStack == null) {
			...
			if (item == null) {
				net.minecraft.server.v1_8_R3.Block block = CraftMagicNumbers.getBlock(itemStack.getType());
				if (block != null) {
					foundType = Block.REGISTRY.c(block).toString();
				}		
			} else {
				foundType = Item.REGISTRY.c(item).toString();
			}
		} else {
			foundType = Item.REGISTRY.c(nmsStack.getItem()).toString();
		}
	...
	public static boolean registerCommand(String command, Plugin plugin, CommandExecutor commandExecutor, String... aliases) {
		try {
	...
	public static boolean unregisterCommand(String command, Plugin plugin) {
		try {
			...
			if (unregisteredCommand != null) unregisteredCommand.unregister(commandMap);
		...
		try {
			...
			if (urlConnection.getResponseCode() == HttpsURLConnection.HTTP_OK) {
				...
				if (username != null) {
					...
					UltimateUtils.notify("Lookup of UUID " + ChatColor.GOLD + uuid.toString() + ChatColor.DARK_GREEN + " to username " + ChatColor.GOLD + username + ChatColor.DARK_GREEN + " succesfully. Saved in usercache.json!");
