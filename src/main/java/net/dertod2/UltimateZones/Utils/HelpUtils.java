...
public class HelpUtils {
	 ...
	 * There are two different methods to allow one global help message and one too define an help message for each argument<br />
	 * This is the first method.
	 * @param sender The Sender which should receive this help message
	 * @param alias The alias of the used command
	 * @param phrase The Phrase of the Command Help
	 */
	 ...
	 * @param alias The alias of the used command
	 * @param phrase The Phrase of the Command Help
	 * @param permission The Permission to check. If the sender hasn't the permission the message won't be sent
	 */
	...
	private static String getCommandToArgument(String label, String command) {
		...
		if (indexOf == -1 || (command.indexOf("[") != -1 && command.indexOf("[") < indexOf)) indexOf = command.indexOf("[");
	...
	private static ChatComponent parse(Locale locale, String alias, String phrase) {
		String commandLine = " " + locale.parse("plain", phrase) + " "; // ugly fix for missing first argument :/ hate regex >.<
		String clickLine = HelpUtils.getCommandToArgument(alias, commandLine);
		
		Pattern pattern = Pattern.compile("([^<][a-zA-Z0-9\\/]*\\s)|(\\[[a-zA-Z0-9\\/\\s]*\\])|(\\[\\<[a-zA-Z0-9\\/\\s]*\\>\\])|(<[a-zA-Z0-9\\/\\s]*>)"); //Pattern.compile("(\\s[a-zA-Z]*\\s)|(\\[[a-zA-Z\\s]*\\])|(\\[<[a-zA-Z\\s]*\\>])|(<[a-zA-Z\\s]*>)");
		
		Matcher matcher = pattern.matcher(commandLine);
		
		ChatComponent chatComponent = new ChatComponent("/" + alias, ChatColor.DARK_GRAY);
		chatComponent.hover(Hover.ShowText, new ChatComponent("/" + alias + " " + commandLine, ChatColor.AQUA).newline().then(locale.parse("plain", phrase + ".descriptions.main"), ChatColor.GRAY));
		...
		while (matcher.find()) {
			...
			if (description == null || description.length() == 0) description = locale.parse("plain", phrase + ".descriptions.main");	// TODO <- use main description?
			
			if (argument.startsWith("<")) { // Importante
				chatComponent.last().then(" ").then(argument, ChatColor.RED)
				.hover(Hover.ShowText, new ChatComponent(argument, ChatColor.AQUA)
				.newline()
				.then(locale.parse("plain", "shared.help.commands.static.important"), ChatColor.DARK_GRAY)
				.newline()
				.then(description, ChatColor.GRAY))
				...
				.hover(Hover.ShowText, new ChatComponent(argument, ChatColor.AQUA)
				.newline()
				.then(locale.parse("plain", "shared.help.commands.static.optional"), ChatColor.DARK_GRAY)
				.newline()
				.then(description, ChatColor.GRAY))
				.click(Click.SuggestCommand, clickLine);
