...
public class ChatComponent {
	...
	private net.minecraft.server.v1_8_R3.NBTTagCompound hoverCompound;
	...
	/**
	 * Creates an new ChatComponent with an hover Effect for the ItemStack data and the given Color Codes
	 * @param itemStack
	...
	public ChatComponent text(ItemStack itemStack) {
		...
		if (nmsStack == null) {
			net.minecraft.server.v1_8_R3.Item item = CraftMagicNumbers.getItem(itemStack.getType());		
			if (item == null) {
				net.minecraft.server.v1_8_R3.Block block = CraftMagicNumbers.getBlock(itemStack.getType());
				if (block == null) {
					this.text(itemStack.getType().name().toLowerCase());
					this.translate(false);
				} else {
					this.text(net.minecraft.server.v1_8_R3.LocaleI18n.get(block.a() + ".name"));
					this.translate(true);
				}		
			} else {
				this.text(net.minecraft.server.v1_8_R3.LocaleI18n.get(item.getName() + ".name"));
				this.translate(true);
			}
		} else {
			this.text(itemMeta.hasDisplayName() ? itemMeta.getDisplayName() : nmsStack.getItem().k(nmsStack) + ".name");
			if (!itemMeta.hasDisplayName()) this.translate(true);
		}
	...
	/**
	 * Enables an hover Action for this ChatComponent when hovering the text with the mouse<br />
	 * The Action is defined in {@link Hover}
	 * @param hoverComponent The Action that will be executed when hovering the text
	 * @param value The Text as an ChatComponent that will be shown or used to render the Action
	 * @return this ChatComponent Object
	 */
	 ...
	 * @param text The Text as an String that will be shown or used to render the Action
	 * @return this ChatComponent Object
	 */
	...
	/**
	 * Enables an ShowText Hover Action for this ChatComponent when hovering the text with the mouse<br />
		...
		if (nmsStack == null) {
			...
			if (item == null) {
				...
				if (block == null) {
					...
					this.hoverCompound = new net.minecraft.server.v1_8_R3.ItemStack(block).save(new net.minecraft.server.v1_8_R3.NBTTagCompound());	
				}		
			} else {
				this.hoverComponent = Hover.ShowItem;	
				this.hoverCompound = new net.minecraft.server.v1_8_R3.ItemStack(item).save(new net.minecraft.server.v1_8_R3.NBTTagCompound());	
			}
		} else {
			this.hoverComponent = Hover.ShowItem;	
			this.hoverCompound = nmsStack.save(new net.minecraft.server.v1_8_R3.NBTTagCompound());	
		}
	...
	/**
	 * Enables an click Action for this ChatComponent when clicking the text with the mouse<br />
	 * The Action is defined in {@link Click}
	 * @param clickComponent The Action that will be executed when clicking on the text
	 * @param value The Text that will be used when clicking on the Text (ig. run Command)
	 * @return this ChatComponent Object
	 */
	 ...
	 * This can be an Color or other Style Codes defined in {@link ChatColor} except RESET
	 * @param chatFormat The Style Code to set
	 * @return this ChatComponent Object
	 */
	 ...
	 * This can be an Color or other Style Codes defined in {@link ChatColor} except RESET<br />
	 * Its useless to set more than one Color but its possible to use more formatting codes
	 * @param chatColor The Style Codes to set
	 * @return this ChatComponent Object
	 */
	 ...
	 * The new Object has its own Text and Style. It can also contain own Hover and Click Actions
	 * @return the new ChatComponent Object
	 */
	...
	/**
	 * Registers an new ChatComponent Object with the given Text after this Object<br />
	 * The new Object has its own Text and Style. It can also contain own Hover and Click Actions
	 * @param text The Text for the new ChatComponent
	 * @return the new ChatComponent Object
	 */
	...
	/**
	 * Registers an new ChatComponent Object with the given Text and Style Codes after this Object<br />
	 * The new Object has its own Text and Style. It can also contain own Hover and Click Actions
	 * @param text
	 * @param chatFormats
	 * @return the new ChatComponent Object
	...
	/**
	 * Adds an new ChatComponent with an hover Effect for the ItemStack data and the given Color Codes
	 * @param itemStack
	 * @return the new ChatComponent object
	 */
	...
	/**
	 * Adds an new ChatComponent Object after this with the ItemStack text and Hover.<br />
	...
	/**
	 * Loops threw all existing ChatComponents referenced with this ChatComponent and returns the last added ChatComponent
	...
	/**
	 * Loops threw all existing ChatComponents referenced with this ChatComponent and return the first created ChatComponent
	...
	/**
	 * Deletes this chatcomponent and all ChatComponents added after this component<br />
	...
	/**
	 * Sends the ChatComponent (automatically uses the first component) to the given {@link CommandSender} object.<br />
	 * When the Receiver is an Player the ChatComponent itself will be send including all extras.<br />
	 * When the Receiver is an console or other objects that don't support ChatComponents the {@link ChatComponent#plain()} Text will be send.
	 * @param player The Sender that receives the message
	 */
	public void to(CommandSender commandSender) {
		...
		if (commandSender instanceof Player) {
			((CraftPlayer) commandSender).getHandle().b(first.server());
		} else {
	...
	/**
	 * Converts an JSON Message to an IChatBaseComponent and sends this to an Player or the Console
	 * @param commandSender
	 * @param json
	 */
	...
	/**
	 * Converts an JSON Message to an IChatBaseComponent and sends this to an Player or the Console<br />
	...
	public static void sendParsed(CommandSender commandSender, String json, ChatComponent extended) {
		IChatBaseComponent iChatBaseComponent = net.minecraft.server.v1_8_R3.IChatBaseComponent.ChatSerializer.a(json);
		if (extended != null) iChatBaseComponent.addSibling(extended.server());
		
		if (commandSender instanceof Player) {
			((CraftPlayer) commandSender).getHandle().b(iChatBaseComponent);
		} else {
	...
	/**
	 * Converts the ChatComponents to normal Chat Messages include Chat Style Codes but no click and hover Objects<br />
	 * This can be used to send this to the console or other objects that don't support ChatComponents
	 * @return
	 */
	public String plain() {
		...
		for (ChatColor chatColor : this.formatList) stringBuilder.append(chatColor);
		stringBuilder.append(this.text);
		
		while (chatComponent.after != null) {
			chatComponent = chatComponent.after;

			for (ChatColor chatColor : chatComponent.formatList) stringBuilder.append(chatColor);	
	 ...
	 */
	public String json() {
		net.minecraft.server.v1_8_R3.IChatBaseComponent chatComponent = this.server();
		return net.minecraft.server.v1_8_R3.IChatBaseComponent.ChatSerializer.a(chatComponent);
	}
	
	/**
	 * Converts this {@link ChatComponent} Object to an Minecraft {@link net.minecraft.server.v1_8_R3.ChatBaseComponent} Object<br />
	 * @return An new Minecraft Source ChatComponent
	 */
	public net.minecraft.server.v1_8_R3.IChatBaseComponent server() {
		net.minecraft.server.v1_8_R3.ChatBaseComponent mainComponent = new net.minecraft.server.v1_8_R3.ChatComponentText("");
		net.minecraft.server.v1_8_R3.IChatBaseComponent workingComponent = null;
		
		net.minecraft.server.v1_8_R3.ChatBaseComponent clearComponent = new net.minecraft.server.v1_8_R3.ChatComponentText("");
		clearComponent.setChatModifier(new net.minecraft.server.v1_8_R3.ChatModifier().setChatModifier(null));
	...
	private net.minecraft.server.v1_8_R3.IChatBaseComponent serverComponent(ChatComponent chatComponent) {
		net.minecraft.server.v1_8_R3.IChatBaseComponent serverComponent = null;
		
		if (chatComponent.mustTranslate) {
			serverComponent = new net.minecraft.server.v1_8_R3.ChatMessage(chatComponent.text); // Translateable Message
		} else { 
			serverComponent = new net.minecraft.server.v1_8_R3.ChatComponentText(chatComponent.text); // Normal message
		}
		
		net.minecraft.server.v1_8_R3.ChatModifier chatModifier = new net.minecraft.server.v1_8_R3.ChatModifier();
		
		if (chatComponent.clickComponent != null) {
			chatModifier.setChatClickable(new net.minecraft.server.v1_8_R3.ChatClickable(chatComponent.clickComponent.getNative(), chatComponent.clickValue));
		}
		
		if (chatComponent.hoverComponent != null) {
			if (chatComponent.hoverComponent == Hover.ShowItem) {
				chatModifier.setChatHoverable(new net.minecraft.server.v1_8_R3.ChatHoverable(chatComponent.hoverComponent.getNative(), new net.minecraft.server.v1_8_R3.ChatComponentText(chatComponent.hoverCompound.toString())));
			} else {
				chatModifier.setChatHoverable(new net.minecraft.server.v1_8_R3.ChatHoverable(chatComponent.hoverComponent.getNative(), chatComponent.hoverValue.server()));
			}
		}
		...
		if (!chatComponent.formatList.isEmpty()) {
			for (ChatColor chatColor : chatComponent.formatList) {
				if (chatColor.isColor()) {
					if (ChatColor.RESET == chatColor) continue;
					chatModifier.setColor(net.minecraft.server.v1_8_R3.EnumChatFormat.b(chatColor.name()));
				} else {
					switch (chatColor) {
					case BOLD:
	...
	/**
	 * Converts this {@link ChatComponent} Object to an Spigot {@link net.md_5.bungee.api.chat.BaseComponent} Object<br />
	...
	private BaseComponent spigotComponent(ChatComponent chatComponent) {
		...
		if (chatComponent.clickComponent != null) {
			baseComponent.setClickEvent(new ClickEvent(chatComponent.clickComponent.getSpigot(), chatComponent.clickValue));
		}
		
		if (chatComponent.hoverComponent != null) {
			if (chatComponent.hoverComponent == Hover.ShowItem) {
				baseComponent.setHoverEvent(new HoverEvent(chatComponent.hoverComponent.getSpigot(), TextComponent.fromLegacyText(chatComponent.hoverCompound.toString())));
			} else {
				baseComponent.setHoverEvent(new HoverEvent(chatComponent.hoverComponent.getSpigot(), chatComponent.hoverValue.spigot()));
			}
		}
	...
	public enum Click {
		OpenFile("open_file", ClickEvent.Action.OPEN_FILE),
		OpenUrl("open_url", ClickEvent.Action.OPEN_URL),
		RunCommand("run_command", ClickEvent.Action.RUN_COMMAND),
		SuggestCommand("suggest_command", ClickEvent.Action.SUGGEST_COMMAND),
		/**
		...
		public net.minecraft.server.v1_8_R3.ChatClickable.EnumClickAction getNative() {
			return net.minecraft.server.v1_8_R3.ChatClickable.EnumClickAction.a(this.code);
		}
	}
	
	public enum Hover {
		ShowText("show_text", HoverEvent.Action.SHOW_TEXT),
		ShowAchievement("show_achievement", HoverEvent.Action.SHOW_ACHIEVEMENT),
		ShowItem("show_item", HoverEvent.Action.SHOW_ITEM),
		/**
		 * Currently not supported.
		 */
		...
		public net.minecraft.server.v1_8_R3.ChatHoverable.EnumHoverAction getNative() {
			return net.minecraft.server.v1_8_R3.ChatHoverable.EnumHoverAction.a(this.code);
		}
	}
}