...
public class MessageUtils {
	...
	public static void sendFailed(AbstractZone abstractZone, Player player, RightResult rightResult) {
		...
		if (rightResult.materialData != null) {
			Locale.json(player, "missingrights.withblock", rightResult.flag.name(), synonym, abstractZone.getId(), abstractZone.getPlayer().getName(), NMSHelper.getMojangIdentifierName(rightResult.materialData.toItemStack()), NMSHelper.getMojangTranslatableName(rightResult.materialData.toItemStack()));
		} else {
			Locale.json(player, "missingrights.withblock", rightResult.flag.name(), synonym, abstractZone.getId(), abstractZone.getPlayer().getName());	
		}
	}
	...
	public static void sendNegatedFailed(AbstractZone abstractZone, Player player, RightResult rightResult) {
		...
		Locale.json(player, "missingrights.negated", rightResult.flag.name(), synonym, abstractZone.getId(), abstractZone.getPlayer().getName());
	}
	
	public static ChatComponent createZoneName(AbstractZone abstractZone) {
		ChatComponent chatComponent;
		if(abstractZone.hasSynonym()){
			chatComponent = new ChatComponent(abstractZone.getSynonym(), ChatColor.GOLD).hover(Hover.ShowText, new ChatComponent("Id: " + abstractZone.getId()));
		} else {
