...
public class PresetCommand implements TabExecutor {
    private final Map<String, String> tabCategoryMap = ImmutableMap.<String, String>builder()
            .put("list", "ultimatezones.commands.preset.list").put("create", "ultimatezones.commands.preset.create")
            .put("edit", "").put("delete", "ultimatezones.commands.preset.delete")
            .put("apply", "ultimatezones.commands.preset.apply").build();
    ...
    private final Map<String, String> tabEditMap = ImmutableMap.<String, String>builder()
            ...
            .put("break", "ultimatezones.commands.preset.edit.material").build();

    /*
     * /preset apply <Preset> <Spieler> [<Zone>] /preset create <Name> [<Spieler>]
     * [<Zone>] /preset delete <Preset> /preset list [<Preset>] /preset edit name
     * <Preset> <Name> /preset edit flag <Preset> <Flag> /preset edit description
     * <Preset> <Description> /preset edit place/break <Preset> <Material>
     */
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
        if (!sender.hasPermission("ultimatezones.commands.preset")) {
            ...
            if ((args.length == 3 || args.length == 4) && args[0].equalsIgnoreCase("apply")
                    && sender.hasPermission("ultimatezones.commands.preset.apply")) {
                this.apply(sender, args);
            } else if ((args.length == 2 || args.length == 3 || args.length == 4) && args[0].equalsIgnoreCase("create")
                    && sender.hasPermission("ultimatezones.commands.preset.create")) {
                this.create(sender, args);
            } else if (args.length == 2 && args[0].equalsIgnoreCase("delete")
                    && sender.hasPermission("ultimatezones.commands.preset.delete")) {
                this.delete(sender, args);
            } else if ((args.length == 1 || args.length == 2) && args[0].equalsIgnoreCase("list")
                    && sender.hasPermission("ultimatezones.commands.preset.list")) {
                this.list(sender, args);
            } else if (args.length == 4 && args[0].equalsIgnoreCase("edit") && args[1].equalsIgnoreCase("name")
                    && sender.hasPermission("ultimatezones.commands.preset.edit.name")) {
                this.editName(sender, args);
            } else if (args.length == 4 && args[0].equalsIgnoreCase("edit") && args[1].equalsIgnoreCase("flag")
                    && sender.hasPermission("ultimatezones.commands.preset.edit.flag")) {
                this.editFlag(sender, args);
            } else if (args.length >= 4 && args[0].equalsIgnoreCase("edit") && args[1].equalsIgnoreCase("description")
                    && sender.hasPermission("ultimatezones.commands.preset.edit.description")) {
                this.editDescription(sender, args);
                    ...
                    && sender.hasPermission("ultimatezones.commands.preset.edit.material")) {
                this.editBlock(sender, args);
            } else {
                HelpUtils.help(sender, alias, "help.commands.preset.apply",
                        new Permission("ultimatezones.commands.preset.apply"));
                HelpUtils.help(sender, alias, "help.commands.preset.create",
                        new Permission("ultimatezones.commands.preset.create"));
                HelpUtils.help(sender, alias, "help.commands.preset.delete",
                        new Permission("ultimatezones.commands.preset.delete"));
                HelpUtils.help(sender, alias, "help.commands.preset.list",
                        new Permission("ultimatezones.commands.preset.list"));
                HelpUtils.help(sender, alias, "help.commands.preset.edit.name",
                        new Permission("ultimatezones.commands.preset.edit.name"));
                HelpUtils.help(sender, alias, "help.commands.preset.edit.flag",
                        new Permission("ultimatezones.commands.preset.edit.flag"));
                HelpUtils.help(sender, alias, "help.commands.preset.edit.description",
                        new Permission("ultimatezones.commands.preset.edit.description"));
                HelpUtils.help(sender, alias, "help.commands.preset.edit.material",
                        new Permission("ultimatezones.commands.preset.edit.material"));
            }
        }
                ...
                if (offlinePlayer != null) {
                    ...
                    if (abstractZone != null) {
                        ...
                        if (rightReference != null) {
                            ...
                            Locale.json(sender, "commands.preset.create.2", args[1], offlinePlayer.getName(),
            ...
            if (presetList.size() > 0) {
                ...
                for (Preset preset : presetList) {
                    chatComponent.last().then(preset.getName(), ChatColor.GOLD)
                            .click(Click.RunCommand, "/preset list " + preset.getName()).then(", ", ChatColor.GRAY);
                }
            ...
            if (material != null) {
                ...
                if (args[1].equalsIgnoreCase("place")) {
                    String phrase = Locale.plain(sender,
                            preset.setPlace(material) ? "shared.added" : "shared.withdrawn");
                    Locale.json(sender, "commands.preset.edit.material.success",
                            NMSHelper.getMojangIdentifierName(itemStack),
                            NMSHelper.getMojangTranslatableName(itemStack), "place", preset.getName(), phrase);
                } else if (args[1].equalsIgnoreCase("break")) {
                    String phrase = Locale.plain(sender,
                            preset.setBreak(material) ? "shared.added" : "shared.withdrawn");
                    Locale.json(sender, "commands.preset.edit.material.success",
                            NMSHelper.getMojangIdentifierName(itemStack),
                            NMSHelper.getMojangTranslatableName(itemStack), "break", preset.getName(), phrase);
                } else {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        } else if (args.length == 2 && (args[0].equalsIgnoreCase("list") || args[0].equalsIgnoreCase("delete")
                || args[0].equalsIgnoreCase("apply"))) {
            List<Preset> presets = Preset.getPresets();
            List<String> presetList = new ArrayList<String>(presets.size());
