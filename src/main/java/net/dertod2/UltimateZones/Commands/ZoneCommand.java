...
public class ZoneCommand implements TabExecutor {
    private final Map<String, String> tabAddMap = ImmutableMap.<String, String>builder().put("main", "")
            .put("independent", "").put("extended", "").build();

    private final Map<String, String> tabCategoryMap = ImmutableMap.<String, String>builder()
            .put("add", "ultimatezones.commands.zone.add").put("info", "ultimatezones.commands.zone.info")
            .put("list", "ultimatezones.commands.zone.list").put("delete", "ultimatezones.commands.zone.delete")
            .put("edit", "").put("display", "ultimatezones.commands.zone.display").build();
    ...
    private final Map<String, String> tabEditMap = ImmutableMap.<String, String>builder()
            ...
            .put("owner", "ultimatezones.commands.zone.edit.owner").build();

    public static Map<UUID, Integer> deleteList = new HashMap<UUID, Integer>();

    /*
     * 
     * TODO -> Set Flags for Zone o.O is missing >.> /zone add main [<Form>]
     * [<3D/2D>] ([<Priority>]) /zone add <Independent/Extended> [<Form>] [<3D/2D>]
     * /zone info [<Zone>] /zone list [<Spieler>] /zone delete [<Zone>] /zone edit
     * <name/priority/owner> <Wert> [<Zone>]
     * 
     * /zone display [<Zone>]
     * 
     * /zone confirm /zone check /zone cancel
     */
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
        if (!sender.hasPermission("ultimatezones.commands.zone")) {
            ...
            if ((args.length >= 2 && args.length <= 5) && args[0].equalsIgnoreCase("add") && sender instanceof Player
                    && sender.hasPermission("ultimatezones.commands.zone.add")) {
                this.add((Player) sender, args);
            } else if ((args.length >= 1 && args.length <= 3) && args[0].equalsIgnoreCase("info")
                    && sender.hasPermission("ultimatezones.commands.zone.info")) {
                this.info(sender, args);
            } else if ((args.length == 1 || args.length == 2) && args[0].equalsIgnoreCase("list")
                    && sender.hasPermission("ultimatezones.commands.zone.list")) {
                this.list(sender, args);
            } else if ((args.length == 1 || args.length == 2) && args[0].equalsIgnoreCase("delete")
                    && sender.hasPermission("ultimatezones.commands.zone.delete")) {
                this.delete(sender, args);
            } else if ((args.length == 3 || args.length == 4) && args[0].equalsIgnoreCase("edit")
                    && args[1].equalsIgnoreCase("name")
                    && sender.hasPermission("ultimatezones.commands.zone.edit.name")) {
                this.editName(sender, args);
            } else if ((args.length == 3 || args.length == 4) && args[0].equalsIgnoreCase("edit")
                    && args[1].equalsIgnoreCase("priority")
                    && sender.hasPermission("ultimatezones.commands.zone.edit.priority")) {
                this.editPriority(sender, args);
            } else if ((args.length == 3 || args.length == 4) && args[0].equalsIgnoreCase("edit")
                    && args[1].equalsIgnoreCase("owner")
                    && sender.hasPermission("ultimatezones.commands.zone.edit.owner")) {
                this.editOwner(sender, args);
            } else if ((args.length == 1 || args.length == 2) && args[0].equalsIgnoreCase("display")
                    && sender instanceof Player && sender.hasPermission("ultimatezones.commands.zone.display")) {
                this.display((Player) sender, args);
            } else if (args.length == 1 && args[0].equalsIgnoreCase("confirm") && sender instanceof Player
                    && sender.hasPermission("ultimatezones.commands.zone.add")) {
            ...
            } else if (args.length == 1 && args[0].equalsIgnoreCase("cancel") && sender instanceof Player
                    && sender.hasPermission("ultimatezones.commands.zone.add")) {
                this.cancel((Player) sender, args);
            } else {
                HelpUtils.help(sender, alias, "help.commands.zone.add.main",
                        new Permission("ultimatezones.commands.zone.add"));
                HelpUtils.help(sender, alias, "help.commands.zone.add.child",
                        new Permission("ultimatezones.commands.zone.add"));
                HelpUtils.help(sender, alias, "help.commands.zone.info",
                        new Permission("ultimatezones.commands.zone.info"));
                HelpUtils.help(sender, alias, "help.commands.zone.list",
                        new Permission("ultimatezones.commands.zone.list"));
                HelpUtils.help(sender, alias, "help.commands.zone.delete",
                        new Permission("ultimatezones.commands.zone.delete"));
                HelpUtils.help(sender, alias, "help.commands.zone.edit");
                HelpUtils.help(sender, alias, "help.commands.zone.display",
                        new Permission("ultimatezones.commands.zone.display"));
            }
        }
    ...
    private void add(Player sender, String[] args) {
        ...
        if (abstractCreation == null) {
            ...
            if (args.length == 4)
                force3D = args[3].equalsIgnoreCase("3D");
            ...
            if (!sender.hasPermission(
                    "ultimatezones.creation." + (ultimateZoneType == UltimateZoneType.MAIN ? "main" : "child") + "."
                            + (force3D ? "3d" : "2d") + "." + zoneType.getName().toLowerCase())) {
                switch (ultimateZoneType) {
                case MAIN:
                    Locale.sendPlain(sender, "commands.zone.add.forbiddentype.main", force3D ? "3D" : "2D",
                            zoneType.getName());
                    break;
                default:
                    Locale.sendPlain(sender, "commands.zone.add.forbiddentype.child", force3D ? "3D" : "2D",
                            zoneType.getName());
                    break;
                }
            ...
            // Check if Player is allowed to create more Main Zones (other Checks only
            // inside Creation Classes)
            if (ultimateZoneType == UltimateZoneType.MAIN
                    && UltimateZones.getConfiguration(sender.getWorld()).getInt("max-zones") != -1) {
                if (!sender.hasPermission("ultimatezones.ignorerestrictions") && AbstractZone.getZones(sender)
                        .size() >= UltimateZones.getConfiguration(sender.getWorld()).getInt("max-zones")) {
                    Locale.sendPlain(sender, "commands.zone.add.error.maxzones", sender.getWorld().getName(),
                            UltimateZones.getConfiguration(sender.getWorld()).getInt("max-zones"));
                    return;
                }
            }

            abstractCreation = new NormalCreation(sender, ultimateZoneType, zoneType, force3D);
            if (args.length == 5 && ultimateZoneType == UltimateZoneType.MAIN
                    && sender.hasPermission("ultimatezones.commands.zone.edit.priority")) {
    ...
    private void info(CommandSender sender, String[] args) {
        ...
        if (abstractZone != null) {
            ...
            DateFormat dateFormat = new SimpleDateFormat(
                    UltimateZones.getConfiguration().getString("date-layouts." + Locale.get(sender).getTag()));

            Locale.json(sender, "commands.zone.info.information",
                    abstractZone.hasSynonym() ? abstractZone.getSynonym() : abstractZone.getId(), abstractZone.getId(),
                    abstractZone.getPlayer().getName(), abstractZone.getType().getName(), zone.getType().getName(),
                    dimension);
            Locale.sendPlain(sender, "commands.zone.info.created", dateFormat.format(zone.getCreated()));
            if (parent != null)
                Locale.json(sender, "commands.zone.info.parent",
                        parent.hasSynonym() ? parent.getSynonym() : parent.getId(), parent.getId(),
                        parent.getPlayer().getName());
            Locale.sendPlain(sender, "commands.zone.info.area_volume",
                    UltimateUtils.formattedArea(zone.getArea(), Locale.get(sender).getJavaLocale()),
                    UltimateUtils.formattedVolume(zone.getVolume(), Locale.get(sender).getJavaLocale()));
    ...
    private void list(CommandSender sender, String[] args) {
        OfflinePlayer offlinePlayer = args.length == 2 ? UltimateUtils.getPlayer(args[1])
        ...
        if (offlinePlayer != null) {
            ...
            for (MainZone mainZone : zoneList) {
                ...
                Locale.json(sender, "commands.zone.list.zone", synonym, mainZone.getId(), offlinePlayer.getName(),
                        childs_zone, childs_recursive);
            }
        }
    }
    ...
    private void delete(CommandSender sender, String[] args) {
        ...
        if (abstractZone != null) {
            if (sender instanceof ConsoleCommandSender || abstractZone.isOwner(sender)
                    || (abstractZone instanceof SubZone && sender instanceof Player && abstractZone
                            .hasRight((Player) sender, Flag.AdministrateZones).rightEnum == RightEnum.Success)) {
                Integer zoneId = ZoneCommand.deleteList
                        .remove(sender instanceof ConsoleCommandSender ? UltimateZones.getUniqueId()
                ...
                if (zoneId != null && abstractZone.getId() == zoneId) {
                    abstractZone.delete();
                    Locale.json(sender, "commands.zone.delete.success", synonym, zoneId,
                            abstractZone.getPlayer().getName(), childs);
                } else {
                    ZoneCommand.deleteList.put(sender instanceof ConsoleCommandSender ? UltimateZones.getUniqueId()
                            : ((Player) sender).getUniqueId(), abstractZone.getId());
                    Locale.json(sender, "commands.zone.delete.question", synonym, abstractZone.getId(),
                            abstractZone.getPlayer().getName(), childs);
                }
            } else {
        ...
        if (abstractZone != null) {
            if (sender instanceof ConsoleCommandSender || abstractZone.isOwner(sender)) {
                if (abstractZone.setSynonym(args[2])) {
                    Locale.sendPlain(sender, "commands.zone.edit.name.success", abstractZone.getId(),
                            abstractZone.getPlayer().getName(), args[2]);
                } else {
        ...
        if (abstractZone != null) {
            if (abstractZone instanceof MainZone) {
                if (UltimateUtils.isInteger(args[2])) {
                    ...
                    String synonym = abstractZone.hasSynonym() ? abstractZone.getSynonym()
                            : String.valueOf(abstractZone.getId());
                    Locale.json(sender, "commands.zone.edit.priority.success", synonym, abstractZone.getId(),
