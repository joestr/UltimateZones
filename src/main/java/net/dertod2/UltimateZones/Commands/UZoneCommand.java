...
public class UZoneCommand implements TabExecutor {
    private final Map<String, String> tabCategoryMap = ImmutableMap.<String, String>builder()
            .put("find", "ultimatezones.commands.find").put("preset", "ultimatezones.commands.preset")
            .put("zone", "ultimatezones.commands.zone").put("rights", "ultimatezones.commands.rights")
            .put("reload", "ultimatezones.commands.reload").put("update", "ultimatezones.updatemessage").build();
        ...
        if (!sender.hasPermission("ultimatezones.commands.main")) {
            ...
            if (args.length != 0) {
                if (args.length == 1 && args[0].equalsIgnoreCase("reload")
                        && sender.hasPermission("ultimatezones.commands.reload")) {
                    this.reload(sender, args);
                    return true;
                } else if (args.length == 1 && args[0].equalsIgnoreCase("update")
                        && sender.hasPermission("ultimatezones.updatemessage")) {
                ...
                if (tabExecutor != null) {
                    tabExecutor.onCommand(sender, command, alias + " " + args[0],
                            args.length > 1 ? Arrays.copyOfRange(args, 1, args.length) : new String[] {});
                } else {
                    Locale.sendPlain(sender, "commands.uzone.notfound", args[0]);
                }
            } else {
                HelpUtils.help(sender, alias, "help.commands.uzone.find",
                        new Permission("ultimatezones.commands.find"));
                HelpUtils.help(sender, alias, "help.commands.uzone.preset",
                        new Permission("ultimatezones.commands.preset"));
                HelpUtils.help(sender, alias, "help.commands.uzone.zone",
                        new Permission("ultimatezones.commands.zone"));
                HelpUtils.help(sender, alias, "help.commands.uzone.rights",
                        new Permission("ultimatezones.commands.rights"));
                HelpUtils.help(sender, alias, "help.commands.uzone.reload",
                        new Permission("ultimatezones.commands.reload"));
                HelpUtils.help(sender, alias, "help.commands.uzone.update",
                        new Permission("ultimatezones.updatemessage"));
            }
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            ...
            TabExecutor tabExecutor = UltimateZones.commandMap.get(args[0].toLowerCase());
            if (tabExecutor != null)
                return tabExecutor.onTabComplete(sender, command, alias + " " + args[0],
